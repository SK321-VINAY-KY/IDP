...
def _run_layer1_2(pdf_path: str) -> list:
    ...
    try:
        ...
        results = process_document(pages_data, vision_llm)
        # process_document returns list[tuple[PageOutput, PageMetadata]]
        # — extract only the PageOutput objects for Layer 3
        return [page_output for page_output, _metadata in results]
    finally:
        doc.close()
...
def _save_to_db(
    ...
    """
    Save extraction run + per-page results. DB failure never breaks the response.
    """
    try:
