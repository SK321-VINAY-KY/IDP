...
PDF_PATH = os.path.join(
    os.path.dirname(__file__), "fixtures", "camscanner_handwritten.pdf"
)
...
def run_pipeline(pdf_path: str) -> tuple[str, list[dict]]:
    ...
    print(
        f"Engine   : PaddleOCR (handwritten mode, det_db_thresh={settings.paddle_handwriting_det_db_thresh})"
    )
    print("Ladder   : digital -> scanned -> handwritten -> vlm_transcribe")
    print(f"{'='*65}\n")
    ...
    for i, page in enumerate(doc):
        ...
        print(
            f"  Page {page_number:2d}: inspected -> route={route} | "
            f"chars={profile.char_count} | img_cov={profile.image_coverage:.2f} | "
            f"scanned={profile.is_scanned}",
            flush=True,
        )
        ...
        print("          running PaddleOCR (handwritten)... ", end="", flush=True)
        ...
        print(
            f"done  confidence={confidence:.3f}  lines={n_lines}  chars_out={len(text.strip())}",
            flush=True,
        )
...
def check_routing_accuracy(page_results: list[dict]) -> None:
    ...
    print(
        f"  Routed as handwritten : {sum(1 for p in page_results if p['route'] == 'handwritten')}"
    )
    print(
        f"  Routed as scanned     : {sum(1 for p in page_results if p['route'] == 'scanned')}"
    )
    ...
    if digital_mismatch:
        for p in digital_mismatch:
            print(
                f"    [FAIL] Page {p['page']} — chars={p['char_count_in_pdf']}, "
    ...
    print(
        f"  Avg image coverage/page : {avg_img_cov:.3f}  (expected >0.25 for scanned)"
    )


def check_content_accuracy(
    page_results: list[dict], full_markdown: str
) -> tuple[int, int]:
    """
...
def check_paddle_output_quality(page_results: list[dict]) -> None:
    ...
    for p in page_results:
        ...
        print(
            f"  {p['page']:>4}  {p['lines_detected']:>5}  {p['paddle_confidence']:>10.3f}  "
    ...
    pages_would_escalate = sum(
        1 for c in confidences if c < settings.escalation_confidence_threshold
    )
    print(
        f"  Pages that would escalate at this threshold: {pages_would_escalate}/{len(confidences)}"
    )
    print(
        "\n  NOTE: PaddleOCR reports genuine per-word scores (typically 0.7-0.99 on legible"
    )
    print(
        "  text). Re-calibrate the threshold based on the distribution above before merge."
    )
...
def main():
    ...
    with open(OUTPUT_MD, "w", encoding="utf-8") as f:
        f.write(
            "# CamScanner Handwritten Notes — Extracted by IDP Pipeline (PaddleOCR handwritten mode)\n\n"
        )
    ...
    if failed == 0:
        ...
        print(
            f"OVERALL: GOOD — {failed} terms missing (handwriting recognition limits expected)"
        )
    elif failed <= 8:
